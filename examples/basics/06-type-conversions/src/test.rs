...
fn test_convert_numbers_overflow() {
    let env = Env::default();
    let result = setup(&env).try_convert_numbers(&i128::MAX, &1u32);
...
fn test_convert_numbers_negative_to_unsigned() {
    let env = Env::default();
    let result = setup(&env).try_convert_numbers(&-100i128, &3u32);
    assert!(result.is_err());
}
...
fn test_create_user_data_name_too_long() {
    ...
    let result = setup(&env).try_create_user_data(&1u64, &long_name, &1000i128, &true);
    assert!(result.is_err());
}
...
fn test_create_user_data_negative_balance() {
    let env = Env::default();
    let name = String::from_str(&env, "alice");
    let result = setup(&env).try_create_user_data(&1u64, &name, &-100i128, &true);
    assert!(result.is_err());
}
...
fn test_convert_val_to_config_missing_field() {
    ...
    let result = setup(&env).try_convert_val_to_config(&val_data);
    assert!(result.is_err());
}
...
fn test_validate_and_convert_number() {
    ...
    let result = setup(&env).validate_and_convert(&input, &1u32);
...
fn test_validate_and_convert_invalid_number() {
    ...
    let result = setup(&env).try_validate_and_convert(&input, &1u32);
    assert!(result.is_err());
}
...
fn test_validate_and_convert_symbol() {
    ...
    let result = setup(&env).validate_and_convert(&input, &2u32);
    assert_eq!(result, input);
}
...
fn test_validate_and_convert_symbol_too_long() {
    ...
    let result = setup(&env).try_validate_and_convert(&input, &2u32);
    assert!(result.is_err());
}
...
fn test_validate_and_convert_address() {
    ...
    let result = setup(&env).validate_and_convert(&input, &3u32);
    assert_eq!(result, input);
}
...
fn test_validate_and_convert_invalid_address() {
    ...
    let result = setup(&env).try_validate_and_convert(&input, &3u32);
    assert!(result.is_err());
}
...
fn test_validate_and_convert_unsupported_type() {
    ...
    let result = setup(&env).try_validate_and_convert(&input, &99u32);
    assert!(result.is_err());
}
...
fn test_batch_convert_numbers_empty_input() {
    ...
    let result = setup(&env).batch_convert_numbers(&input_vec);
    assert_eq!(result.len(), 0);
}
...
fn test_sum_different_types() {
    let env = Env::default();
    let result = setup(&env).sum_different_types(&100u32, &-50i64);
...
fn test_val_conversion_roundtrip_via_safe_conversions() {
    ...
    let (success, converted) = setup(&env).safe_conversions(&val, &1u32);
    assert!(success);
